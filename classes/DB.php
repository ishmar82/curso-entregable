	...
	abstract class DB
	{
		...
		public static function getAllActors(){
			...
			$stmt  = $connection->prepare("
				SELECT a.id AS 'actor_id', a.first_name AS 'nombre', a.last_name AS 'apellido', a.rating, m.title AS 'fav'
				FROM actors as a
			...
			foreach ($actors as $actor) {
				$finalActor = new Actor($actor['nombre'], $actor['apellido'], $actor['rating'], $actor['fav']);

				$finalActor->setActorId($actor['actor_id']);
				$finalActor->setFavoriteMovie($actor['fav']);
		...
		public static function saveActor(Actor $actor){
			...
			try {
				$stmt = $connection->prepare("
					...
					VALUES(:nombre, :apellido, :rating, :fav_movie_id)
					");

				$stmt->bindValue(':nombre', $actor->getFirstName());
				$stmt->bindValue(':apellido', $actor->getLastName());
				$stmt->bindValue(':rating', $actor->getRating());
				$stmt->bindValue(':fav_movie_id', $actor->getFavoriteMovie());
